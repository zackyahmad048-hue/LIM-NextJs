...
export function hijriMonthName(month: number, locale: "en" | "id" = "id"): string {
  const names = locale === "id" ? HIJRI_MONTH_NAMES_ID : HIJRI_MONTH_NAMES;
  return names[month - 1] ?? "";
}

export function hijriDateToString(hijri: HijriDate, locale: "en" | "id" = "id"): string {
...
export function getDaysInHijriMonth(year: number, month: number): number {
  ...
  const isLeapYear = year % 30 === 2 || year % 30 === 5 || year % 30 === 7 ||
