...
const BASE_URL = process.env.E2E_BASE_URL ?? "https://lim-next-js.vercel.app";
const EMAIL = process.env.ADMIN_EMAIL;
const PASSWORD = process.env.ADMIN_PASSWORD;

describe("regresi alur login + surat QR (produksi)", () => {
  it("login -> terbitkan surat -> verifikasi publik -> bersihkan", async () => {
    ...
    try {
      ...
      expect(await iframe.getAttribute("src")).toContain(
        "/api/v1/verifikasi/surat/",
      );
      expect(await iframe.getAttribute("src")).toContain("/file");
  ...
  it("validasi jenis file: gambar berpratinjau, teks & PDF rusak ditolak", async () => {
    expect(EMAIL, "ADMIN_EMAIL harus diisi di .env").toBeTruthy();
    expect(PASSWORD, "ADMIN_PASSWORD harus diisi di .env").toBeTruthy();
    ...
    try {
      ...
      await expect
        .poll(
          async () =>
            await page.locator('img[alt^="Pratinjau"]').getAttribute("src"),
          { timeout: 30_000 },
        )
        .toContain("/api/v1/verifikasi/surat/");
      ...
      await expect
        .poll(
          async () =>
            (await page.locator("li", { hasText: txtNumber }).count()) > 0,
      ...
      await expect
        .poll(
          async () =>
            (await page.locator("li", { hasText: corruptNumber }).count()) > 0,
          { timeout: 60_000, interval: 1000 },
        )
        .toBe(false);
