...
import { updateIncomingMail, transitionIncomingMailStatus } from "@/modules/secretariat/presentation/secretariat.action";
...
export default async function EditIncomingMailPage({
  ...
  if (mail.status === "RECEIVED") statusActions.push({ label: "Proses", status: "PROCESSED", icon: CheckCircle, variant: "default" });
  if (mail.status === "PROCESSED") statusActions.push({ label: "Arsipkan", status: "ARCHIVED", icon: Archive, variant: "outline" });
    ...
    <PageContainer>
      ...
      <div className="flex flex-wrap gap-2">
        {statusActions.map((action) => {
          ...
          return (
            <form key={action.status} action={transitionIncomingMailStatus.bind(null, mail.id, action.status)}>
      ...
      <form action={updateIncomingMail.bind(null, mail.id)} className="max-w-2xl space-y-3">
        <SectionCard className="rounded-lg p-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="receivedDate" name="receivedDate" type="date" required defaultValue={formatDateInput(mail.receivedDate)} className="rounded-md text-xs" />
            </div>

            <div className="space-y-1.5 md:col-span-2">
              <Label htmlFor="sender" className="text-xs">Pengirim</Label>
              <Input id="sender" name="sender" required defaultValue={mail.sender} className="rounded-md text-xs" />
            </div>

            <div className="space-y-1.5 md:col-span-2">
              <Label htmlFor="senderAddress" className="text-xs">Alamat Pengirim</Label>
              <Input id="senderAddress" name="senderAddress" defaultValue={mail.senderAddress ?? ""} className="rounded-md text-xs" />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input id="classification" name="classification" defaultValue={mail.classification ?? ""} className="rounded-md text-xs" />
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="category" className="text-xs">Kategori</Label>
              <Input id="category" name="category" defaultValue={mail.category ?? ""} className="rounded-md text-xs" />
            </div>

            <div className="space-y-1.5 md:col-span-2">
              <Label htmlFor="notes" className="text-xs">Catatan</Label>
              <Textarea id="notes" name="notes" defaultValue={mail.notes ?? ""} className="min-h-20 rounded-md text-xs" />
            </div>
