...
export function SuratPageClient({
  ...
  const statusLabels =
    tab === "masuk" ? incomingStatusLabels : outgoingStatusLabels;

  async function handleDelete(id: string) {
    const confirmDelete = window.confirm("Hapus surat ini?");
    if (!confirmDelete) return;

    const { deleteIncomingMail, deleteOutgoingMail } =
      await import("@/modules/secretariat/presentation/secretariat.action");
