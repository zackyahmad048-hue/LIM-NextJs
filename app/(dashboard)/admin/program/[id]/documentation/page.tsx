...
import { getProgramById, getDocumentation } from "@/modules/program/queries/program.query";
import { addDocumentation, removeDocumentation } from "@/modules/program/presentation/program.action";
...
export default async function DocumentationPage({
  ...
  const [program, documentation] = await Promise.all([getProgramById(id), getDocumentation(id)]);
  ...
  return (
    <PageContainer>
      ...
      <form action={addDocumentation.bind(null, program.id)} className="max-w-sm space-y-3 rounded-lg border bg-background p-4">
        ...
        <div className="space-y-1.5">
          ...
          <input name="mediaId" required className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-xs shadow-sm" />
        </div>
        <div className="space-y-1.5">
      ...
      <AdminTable
        ...
        columns={[
          ...
          },
          {
            ...
            align: "right",
            render: (item) => (
              <form action={removeDocumentation.bind(null, item.id, program.id)}>
