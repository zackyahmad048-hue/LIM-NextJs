...
export function getTimezoneFromLongitude(longitude: number): { timezone: number; timezoneName: string } {
  if (longitude >= 127.5) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        if (error.code === error.PERMISSION_DENIED) {
          msg = 'Akses lokasi ditolak oleh pengguna. Menggunakan lokasi default (Jakarta).';
